...
title: str = md.Meta['title'][0] # type: ignore
created: str = f"created: {md.Meta['created'][0]}" # type: ignore

data = (
  data
  .replace('<siteContentBlock />', content)
  .replace('<postTitleBlock />', title)
  .replace('<postCreatedBlock />', created)
)

with open('compiled_index.html', 'w') as file:
    file.write(data)